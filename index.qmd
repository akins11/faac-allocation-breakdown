...
```{r}
curr_all_info <- get_current_year_data(state_df, lgc_df, previous_yr = FALSE)
...
```{r}
current_date_info <- get_last_faac_month(state_df, lgc_df)
...
```{r}
# analysis_yr_range <- analysis_full_year_range(state_df, lga_df)
```
...
```{r}
curr_nav_year <- current_year 
prev_nav_year <- current_year-1 
